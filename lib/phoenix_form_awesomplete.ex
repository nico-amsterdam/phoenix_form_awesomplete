defmodule PhoenixFormAwesomplete do
  ...
  defp chars_to_string(text) do
    if is_list(text), do: :erlang.list_to_binary(text), else: text
  end
  ...
  def copy_to_id(source_form, source_field, data_field \\ nil, target_id) 
    ...
    script(copy_to_id_script(source_form, source_field, data_field, target_id))
  end
  ...
  def awesomplete(form, field, opts \\ [], awesomplete_opts) do
    ...
    # 
      ...
      # For multiple items, only the last item is considered to be input for the suggestion list.
    # 
    conv_input_str = 
    ...
    data_val = if is_nil(fld_name) or is_nil(descr_fld), do: "data", else: "data.value"
    starts_with = filter_fun == "Awesomplete.FILTER_STARTSWITH" or filter_fun == @util <> ".filterStartsWith"
    starts_with_filter_fun = if is_nil(descr_fld) and is_nil(label_fld) and maxItems !== 0 and filter_fun == "Awesomplete.FILTER_STARTSWITH", do: "Awesomplete.FILTER_STARTSWITH", else: @util <> ".filterStartsWith"
    ...
    filter_opts = cond do
      ...
      is_nil(multiple_char) and (is_nil(filter_fun) or filter_fun == "Awesomplete.FILTER_CONTAINS" or filter_fun == @util <> ".filterContains") -> [filter: @util <> ".filterContains"]
      is_nil(multiple_char) and data_val == "data" -> [filter: filter_fun]
      is_nil(multiple_char) and starts_with and Keyword.has_key?(awesomplete_opts, :item) -> [filter: starts_with_filter_fun]
      is_nil(multiple_char) and starts_with -> [filter: starts_with_filter_fun , item: @util <> ".itemStartsWith"]
      is_nil(multiple_char) -> [filter: "function(data, input) { return (#{filter_fun})(data.value, input); }"]
      is_nil(filter_fun) -> [filter: "function(data, input) { return Awesomplete.FILTER_CONTAINS(#{data_val}, input.match(/[^#{multiple_char}]*#{filter_match_sep}$/)[0]); }"]
      true -> [filter: "function(data, input) { return (#{filter_fun})(#{data_val}, #{conv_input_str}); }"]
    end 
